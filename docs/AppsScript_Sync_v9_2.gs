/**
 * BarAI Desktop v9.2 - Sync cloud + fogli Google leggibili
 *
 * Uso:
 ...
 *    BARAI_SYNC_FOLDER_ID = cartella Drive dove salvare BarAI_Cloud_Snapshot_v9_2.json.
...
const BARAI_SYNC_FILE_NAME = 'BarAI_Cloud_Snapshot_v9_2.json';
...
const SHEET_DEFS = [
  ...
  { sheet: 'Fatture_Vendita', table: 'fatture_vendita', columns: ['id','cliente_nome','cliente_partita_iva','cliente_codice_fiscale','numero','data_fattura','imponibile','iva','totale','valuta','stato','data_incasso','origine_import','xml_nome_file','note','created_at','updated_at'] },
  { sheet: 'Fatture_Vendita_Righe', table: 'fatture_vendita_righe', columns: ['id','fattura_id','numero_linea','codice_articolo','descrizione_originale','quantita','um','prezzo_unitario','totale_riga','aliquota_iva','created_at','updated_at'] },
...
function getSnapshotFile_() {
  ...
  const legacy911 = folder.getFilesByName('BarAI_Cloud_Snapshot_v9_1_1.json');
  if (legacy911.hasNext()) return legacy911.next();
  const legacy90 = folder.getFilesByName('BarAI_Cloud_Snapshot_v9_0.json');
  if (legacy90.hasNext()) return legacy90.next();
